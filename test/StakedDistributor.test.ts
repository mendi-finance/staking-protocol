...
async function deployTokensFixture() {
    ...
    // Mendi
    const Mendi = await ethers.getContractFactory("MockERC20Token");
    const mendi = await Mendi.connect(admin).deploy(
        ethers.utils.parseUnits("10000", 18),
        18
    );

    // Send some Mendi to the staker0 and staker1
    await (
        await mendi
            .connect(admin)
            .transfer(staker0.address, ethers.utils.parseUnits("10", 18))
    ).wait(1);
    await (
        await mendi
            .connect(admin)
            .transfer(staker1.address, ethers.utils.parseUnits("10", 18))
    ).wait(1);

    // reward0
    const MockERC20Token = await ethers.getContractFactory("MockERC20Token");
    const reward0 = await MockERC20Token.connect(rewardHolder).deploy(
        ethers.utils.parseUnits("10000", 18),
        18
    );
    const reward1 = await MockERC20Token.connect(rewardHolder).deploy(
        ethers.utils.parseUnits("10000", 18),
        18
    );
...
describe("Staked Distributor", function () {
    ...
    it("Should withdraw burned tokens", async function () {
        ...
        expect(await mendi.balanceOf(staker0.address)).to.be.equal(
            staker0Mendi.add(toBurn0)
        );
        ...
        // Burn staked tokens and withdraw Mendi: Staker1
        const staker1Mendi = await mendi.balanceOf(staker1.address);
        ...
        expect(await mendi.balanceOf(staker1.address)).to.be.equal(
            staker1Mendi.add(toBurn1)
        );
    });
    ...
    it("Should claim rewards", async function () {
        ...
        expect(await reward1.balanceOf(stakedDistributor.address)).to.be.equal(
            totalRewardAmountsByPool[1]
        );

        // Claim first staker second time
        const reward0_staker0_balance = await reward0.balanceOf(
            staker0.address
        );
        ...
        const reward0_staker0_balance2 = await reward0.balanceOf(
            staker0.address
        );
        expect(reward0_staker0_balance2).to.be.equal(
            reward0staker0_0.add(reward0staker0_1)
        );
        expect(reward0_staker0_claimable).to.be.equal(
            reward0_staker0_balance2.sub(reward0_staker0_balance)
        );

        // Claim All second staker for two distribution
        const reward0_staker1_balance = await reward0.balanceOf(
            staker1.address
        );
        ...
        const reward1_staker1_balance = await reward1.balanceOf(
            staker1.address
        );
        ...
        // Check reward0
        const claimedstaker1reward0_total = await reward0.balanceOf(
            staker1.address
        );
        expect(claimedstaker1reward0_total).to.be.equal(
            reward0staker1_0.add(reward0staker1_1)
        );
        expect(reward0_staker1_claimable).to.be.equal(
            claimedstaker1reward0_total.sub(reward0_staker1_balance)
        );

        // Check reward1
        const claimedstaker1reward1_total = await reward1.balanceOf(
            staker1.address
        );
        expect(claimedstaker1reward1_total).to.be.equal(
            reward1staker1_0.add(reward1staker1_1)
        );
        expect(reward1_staker1_claimable).to.be.equal(
            claimedstaker1reward1_total.sub(reward1_staker1_balance)
        );
    });
