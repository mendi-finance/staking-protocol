...
let mendiAddress = "0x1db2466d9f5e10d7090e7152b68d62703a2245f0";
...
async function deployTokensFixture() {
    ...
    // Give participants some Mendi
    const initialBalance0 = ethers.utils.parseEther("1500");
    const initialBalance1 = ethers.utils.parseEther("5000");
    await (
        await mendi.connect(admin).transfer(staker0.address, initialBalance0)
    ).wait(1);
    await (
        await mendi.connect(admin).transfer(staker1.address, initialBalance1)
    ).wait(1);
